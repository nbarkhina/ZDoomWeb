...
#define DEFAULT_SOUNDFONT "/soundfonts/gzdoom.sf2"
#else
...
/* The Emscripten/WebAssembly build is single-threaded (no pthreads /
   SharedArrayBuffer) and drives FluidSynth synchronously, so the mixer must
...
/* Define if using glib stubs instead of real glib. The Win32 build uses the
   bundled win32_glibstubs; the Emscripten/wasm build uses the portable,
   single-threaded emscripten_glibstubs (real glib is unavailable for wasm). */
