...
// "Low Memory Mode" toggle. Set once at startup from config.txt (see
// D_ReadStartupConfig). When true, the renderer allocates smaller flat-vertex
// pipeline buffers and fewer of them to roughly halve the wasm heap (see
// memory.md). Off by default so the normal, higher-memory behaviour is unchanged.
...
public:
	...
	// Size (in vertices) of each persistent flat-vertex pipeline buffer. Chosen in
	// the constructor: 2,000,000 verts (61 MB each) normally, or 500,000 (15 MB
	// each) when Low Memory Mode is on (gl_low_memory), which is ample for Doom
	// maps and roughly halves the wasm heap. See memory.md. These are runtime
	// values (not compile-time constants) so the setting can gate them per launch.
	unsigned int mBufferSize;
	unsigned int mBufferSizeToUse;
