...
namespace OpenGLESRenderer
{
	...
	void InitGLES()
	{
		...
		// GLES reports e.g. "OpenGL ES 3.0 (WebGL 2.0 ...)" -- skip any leading
		// text so strtod() sees the actual version number (otherwise it returns 0).
		const char* glVersionNum = glVersionStr;
		while (*glVersionNum && (*glVersionNum < '0' || *glVersionNum > '9'))
			glVersionNum++;
		double glVersion = strtod(glVersionNum, NULL);
		...
		if (gles.glesMode == GLES_MODE_GLES)
		{
			...
			// WebGL2 provides GLSL ES 3.00, which (unlike ES 1.00) permits dynamic
			// indexing of uniform arrays -- required by the dynamic-light shaders.
			// Compatibility #defines for the ES 1.00-style source are injected at
			// shader assembly time (gles_shader.cpp / gles_shaderprogram.cpp).
			gles.shaderVersionString = "300 es";
#else
			gles.shaderVersionString = "100";
#endif
			// On GLES 3.0+ (e.g. WebGL2) packed depth/stencil and NPOT textures
			// are core features, so the OES extension strings are not advertised.
			// Treat them as available to avoid an incomplete framebuffer.
			gles.depthStencilAvailable = (glVersion >= 3.0) || CheckExtension("GL_OES_packed_depth_stencil");
