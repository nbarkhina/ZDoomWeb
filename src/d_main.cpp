...
// Coalescing save-backup timer (i_savestate_web.cpp): ticked once per frame.
...
static int d_loop_lasttic = 0;

// One iteration of the play loop. Returns false when the loop should exit
// (a 'restart' CCMD was issued). Split out so the Emscripten build can drive it
// from emscripten_set_main_loop -- a single-threaded, cooperative, per-frame
// scheme that is exception-safe (unlike ASYNCIFY) and needs no pthreads.
static bool D_DoomLoopIteration ()
{
	try
	{
		...
		if (gametic > d_loop_lasttic)
		{
			d_loop_lasttic = gametic;
			I_StartFrame ();
		}
		I_SetFrameTime();
...
#ifdef __EMSCRIPTEN__
static void D_DoomLoopFrame ()
{
	// Tick the coalescing save-backup timer once per rendered frame (see
	// i_savestate_web.cpp); it fires the JS auto-backup when it expires.
	Neil_TickSaveBackup();

	if (!D_DoomLoopIteration())
...
#ifdef __EMSCRIPTEN__
	// The engine is fully initialised and about to enter the main loop: tell the
	// JS front-end so it can auto-restore the backed-up save games (from
	// IndexedDB or the cloud) into /Save before the player opens the load menu.
	EM_ASM({
		if (window.myApp && window.myApp.onGameReady)
			window.myApp.onGameReady();
	});

	// Cooperative, single-threaded loop: the browser drives one frame per
	// animation frame (fps == 0 -> requestAnimationFrame). The trailing 1
	// (simulate_infinite_loop) unwinds the C stack but keeps the runtime alive,
	// so this call does not return -- which matches the desktop loop, which also
	// only returns here on a 'restart'.
	emscripten_set_main_loop(D_DoomLoopFrame, 0, 1);
	printf("Emscripten main loop exited, which should never happen.\n");
#else
	for (;;)
	{
		if (!D_DoomLoopIteration())
...
// D_ReadStartupConfig
//
// Reads config.txt before the renderer is created and applies the handful of
// settings that must be known that early. The file is written by the web front-
// end (script.js writeConfigTxt(), mirroring the DosBox-X pattern) or can be
// dropped next to the exe on desktop. Line-based, one integer per line:
//   line 0: Low Memory Mode (1/0) -> gl_low_memory
// An absent/unreadable file leaves every setting at its default. progdir is "/"
// on the web build, so this reads /config.txt there; on Windows it reads
// config.txt next to the executable.
//
//==========================================================================

extern bool gl_low_memory;

static void D_ReadStartupConfig()
{
	FString path = progdir;
	path += "config.txt";
	...
	while (fgets(line, sizeof(line), f) != nullptr)
	{
		...
		switch (idx)
		{
		case 0: gl_low_memory = (v != 0); break;	// "Low Memory Mode"
		default: break;
		}
		idx++;
	}
	...
	if (gl_low_memory)
		Printf("Low Memory Mode enabled (config.txt): using smaller renderer vertex buffers.\n");
}
