...
// Coalescing save-backup timer (i_savestate_web.cpp): arm on a successful save.
...
#ifdef __EMSCRIPTEN__
		// Auto-back-up the save that just landed on the MEMFS /Save tree. Hand it
		// to the arming function (i_savestate_web.cpp), which applies the policy:
		//   * logged in  -> only a deliberate menu save backs up, immediately;
		//   * logged out -> every save backs up, coalesced via a frame countdown.
		// The two flags identify the save source at this single choke point that
		// every save funnels through:
		//     autosave           (G_DoAutoSave)  -> (okForQuicksave=false, forceQuicksave=false)
		//     menu / named save  (ga_savegame)   -> (true,  false)
		//     rotating quicksave (G_DoQuickSave) -> (true,  true)
		// (A fixed-slot quicksave shares the menu-save path -> (true,false), so it
		// counts as a menu save; there's no way to tell them apart at this layer
		// and it's an equally deliberate save, so that's fine.)
		const int isMenuSave = (okForQuicksave && !forceQuicksave) ? 1 : 0;
		Neil_ArmSaveBackup(isMenuSave);
#endif
	}
	else
	{
