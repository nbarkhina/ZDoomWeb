...
// The single-threaded web build has no pthreads: keep the BSP renderer on the
// synchronous path (gl_multithread default false) and give the pool 0 worker
// threads so its global constructor never tries to spawn a std::thread.
...
#ifdef __EMSCRIPTEN__
ctpl::thread_pool renderPool(0);   // no worker thread in the single-threaded web build
#else
