...
#ifdef __EMSCRIPTEN__
	// The web MEMFS is wiped on reload, so whenever the engine writes the default
	// config (the "Save config" options-menu item / the writeini CCMD / a restart)
	// tell the JS front-end to persist the freshly-written gzdoom.ini to
	// IndexedDB. Only the default config is persisted (filename == nullptr); an
	// explicit "writeini <somefile>" to another path is left alone.
	if (filename == nullptr && success)
	{
		EM_ASM({
			if (window.myApp && window.myApp.onConfigWritten)
				window.myApp.onConfigWritten();
		});
	}
#endif
	return success;
